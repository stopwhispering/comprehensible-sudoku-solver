...
def find_xy_wing(board: Board):
    ...
    be in the same house (cf. naked triple). As either one or both of the pincers have value Z, we may invalidate Z
    ...
    for pivot_cell in starting_cells:
        candidate_x, candidate_y = pivot_cell.possible_values
        ...
        for pincer_combination in pincer_combinations:
            ...
            candidate_z = [c for c in pincer_combination[0].possible_values if c != candidate_x][0]
            candidate_z_b = [c for c in pincer_combination[1].possible_values if c != candidate_y][0]
...
def find_xyz_wing(board: Board):
    ...
    for pivot_cell in starting_cells:
        ...
        for candidate_z in pivot_cell.possible_values:

            candidate_x, candidate_y = [c for c in pivot_cell.possible_values if c != candidate_z]
